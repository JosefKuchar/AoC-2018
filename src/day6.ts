    ...
    for (let x = 0; x < 500; x++) {
        for (let y = 0; y < 500; y++) {
            ...
            coordinates.forEach(coordinate => {
                dst += distance(x, y, coordinate[0], coordinate[1]);
            });
...
export function solve(input: string, lessThan: number = 10000) {
    ...
    let grid: number[][] = new Array(500)
        .fill(0)
        .map(x => new Array(500).fill(-1));
    let finite = new Set();
    ...
    for (let i = 0; i < 500; i++) {
        ...
        finite.delete(closest(coordinates, i, 500));
        finite.delete(closest(coordinates, 0, i));
        finite.delete(closest(coordinates, 500, i));
    }
    ...
    return {
        part1: areas.reduce((acc, val) => (val > acc ? acc = val : acc)),
